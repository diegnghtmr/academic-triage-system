...
public class AttendRequestService implements AttendRequestUseCase {
    ...
    public AttendRequestService(LoadRequestForMutationPort loadRequestForMutationPort,
    ...
    @Override
    public RequestSummary execute(AttendRequestCommand command, AuthenticatedActor actor) {
        ...
        request.attend(command.observations(), actor.userId(), LocalDateTime.now());
    ...
    private void ensureStaffActor(AuthenticatedActor actor) {
        if (actor.role() != Role.STAFF) {
            throw new UnauthorizedOperationException(actor.role(), "attend request");
        }
    }
}
