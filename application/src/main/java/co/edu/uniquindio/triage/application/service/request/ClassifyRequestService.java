...
public class ClassifyRequestService implements ClassifyRequestUseCase {
    ...
    public ClassifyRequestService(LoadRequestForMutationPort loadRequestForMutationPort,
    ...
    @Override
    public RequestSummary execute(ClassifyRequestCommand command, AuthenticatedActor actor) {
        ...
        var requestType = loadRequestTypePort.loadById(command.requestTypeId())
                .filter(co.edu.uniquindio.triage.domain.model.RequestType::isActive)
                .orElseThrow(() -> new IllegalArgumentException("El tipo de solicitud no existe o está inactivo"));

        request.classify(requestType.getId(), command.observations(), actor.userId(), LocalDateTime.now());
    ...
    private void ensureStaffActor(AuthenticatedActor actor) {
        if (actor.role() != Role.STAFF) {
            throw new UnauthorizedOperationException(actor.role(), "classify request");
        }
    }
}
